...
from mmdet.datasets.pipelines import RandomFlip, LoadImageFromFile
from . import Compose
...
class PadMultiViewImage_petrv2(object):
...
@PIPELINES.register_module()
class NormalizeMultiviewImage_petrv2(object):
    """Normalize the image.
...
@PIPELINES.register_module()
class ResizeCropFlipImage_petrv2(object):
    ...
    def __call__(self, results):
        ...
        for i in range(N):
            ...
            intrinsics = np.eye(4)
            intrinsics[:3, :3] = ida_mat @ results['intrinsics'][i][:3, :3]
            results['intrinsics'][i] = intrinsics 
    ...
    def _img_transform(self, img, resize, resize_dims, crop, flip, rotate):
        ...
        if flip:
            ...
            ida_tran = A.matmul(ida_tran) + b # x = w - x
        A = self._get_rot(rotate / 180 * np.pi) # 绕z轴旋转rotate度的2x2旋转矩阵
...
@PIPELINES.register_module()
class GlobalRotScaleTransImage_petrv2(object):
    """Random resize, Crop and flip the image
    Args:
        size (tuple, optional): Fixed padding size.
    """
    ...
    def __call__(self, results):
        ...
        """
        # random rotate
        rot_angle = np.random.uniform(*self.rot_range) # ±22.5°之间随机选
    ...
    def rotate_bev_along_z(self, results, angle):
        ...
        rot_mat_inv = torch.inverse(rot_mat) # 绕z轴旋转angle角的4x4齐次变换矩阵
