    ...
    for i in range(num_rects):
        ...
        for start, end in line_indices:
            cv2.line(img, (corners[start, 0], corners[start, 1]),
                     ...
                     cv2.LINE_AA)
        if isinstance(labels3d, np.ndarray):
            label_point = (int((max(0, corners[1][0])+max(0, corners[7][0]))/2), 
                           int((max(0, corners[1][1])+max(0, corners[7][1]))/2))
            if isinstance(scores3d, np.ndarray):
                cv2.putText(img, str(labels3d[i])+':'+str(round(scores3d[i], 3)), label_point, cv2.FONT_HERSHEY_DUPLEX,
                            1, color, thickness, cv2.LINE_AA)
            else:
                cv2.putText(img, str(labels3d[i]), label_point, cv2.FONT_HERSHEY_DUPLEX,
                            1, color, thickness, cv2.LINE_AA)
        for start, end in heading_line_indices:
            cv2.line(img, (corners[start, 0], corners[start, 1]),
                     (corners[end, 0], corners[end, 1]), (0, 255, 255), thickness,
                     cv2.LINE_AA)
...
def draw_camera_bbox3d_on_img(bboxes3d,
    ...
    return plot_rect3d_on_img(img, num_bbox, imgfov_pts_2d, labels3d, scores3d, color, thickness)
