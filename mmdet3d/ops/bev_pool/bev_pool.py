...
def bev_pool(feats, coords, B, D, H, W):
    ...
    feats, coords, ranks = feats[indices], coords[indices], ranks[indices] # 排序

    x = QuickCumsumCuda.apply(feats, coords, ranks, B, D, H, W) #[B, D, H, W, 64]
    x = x.permute(0, 4, 1, 2, 3).contiguous() # [B, 64, D, H, W]
    return x
